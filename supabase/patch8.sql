-- ============================================================================
-- JIO CLUB v7 — PATCH 8 (idempotent; safe to re-run)
-- 9 security/logic hardening fixes. All changes are CREATE OR REPLACE /
-- DROP POLICY IF EXISTS / REVOKE+GRANT — no table drops, no feature loss.
--
--  Issue 1 (CRITICAL) settle_period        : + is_admin() second-layer guard
--                                            (current_user service-only guard
--                                            kept so pg_cron context works)
--  Issue 2 (HIGH)     pick_result, tick_game: EXECUTE revoked from
--                                            anon/authenticated/public
--  Issue 3 (HIGH)     place_bet             : strict selection validation
--  Issue 4 (HIGH)     notify_admin          : service-role-only (revoke client)
--  Issue 5 (MEDIUM)   razorpay credit       : atomic idempotent
--                                            public.razorpay_credit re-asserted
--                                            (edge fn v4 already calls it — this
--                                            is the requested atomic credit)
--  Issue 6 (MEDIUM)   mfa_check_code        : rate limiting (5 fails/5min =>
--                                            15min lockout, reset on success)
--  Issue 7 (MEDIUM)   deposits_insert_own   : RLS INSERT policy DROPPED —
--                                            request_deposit() only
--  Issue 8 (MEDIUM)   self-exclusion        : OFF deferred — 24-72h cooldown
--                                            + admin_action('lift-self-exclusion')
--  Issue 9 (LOW)      function surface      : global revoke + explicit client
--                                            grant list; adjust-balance atomic
--
...
-- ----------------------------------------------------------------------------
-- Issue 8 prelude: additive columns + settings seed
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- Issue 6 prelude: mfa_attempts table (RLS on, NO policies = definer-only)
-- ----------------------------------------------------------------------------
...
-- NOTE: intentionally NO policy — only SECURITY DEFINER functions (as owner)
-- read/write this table.


-- ----------------------------------------------------------------------------
-- Issue 1: settle_period (hardened + is_admin second layer)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- Issue 2: pick_result, tick_game (service-only, re-asserted)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- Issue 3: place_bet (+ selection validation)
-- ----------------------------------------------------------------------------
...
begin
  ...
  -- PATCH8 (issue 3): strict selection validation — settle_period kabhi
  -- unparseable selection receive nahi karega
  if p_type = 'number' then
    if p_selection is null or p_selection !~ '^[0-9]$' then
      raise exception 'invalid selection for number bet (must be a single digit 0-9)';
    end if;
  elsif p_type = 'color' then
    if p_selection not in ('Green','Red','Violet') then
      raise exception 'invalid selection for color bet (Green/Red/Violet)';
    end if;
  elsif p_type = 'size' then
    if p_selection not in ('Big','Small') then
      raise exception 'invalid selection for size bet (Big/Small)';
    end if;
  end if;
...
-- ----------------------------------------------------------------------------
-- Issue 4: notify_admin (body unchanged; grants locked below)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- Issue 5: razorpay_credit (atomic, idempotent — re-asserted)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- Issue 6: mfa_check_code (+ rate limiting)
-- ----------------------------------------------------------------------------
...
begin
  ...
  -- PATCH8 (issue 6): rate limiting — 5 failed attempts in a 5-minute window
  -- => 15-minute lockout; count resets on success or window expiry.
  ...
  -- failed attempt: increment (window rolls after 5 min)
...
-- ----------------------------------------------------------------------------
-- Issue 8: toggle_self_exclusion (cooldown + admin lift)
-- ----------------------------------------------------------------------------
...
begin
  ...
  -- turning ON stays instant (as before)
  if not v_ex then
    update public.profiles
  ...
  -- PATCH8 (issue 8): turning OFF is DEFERRED — 24-72h cooldown (settings key
  -- 'self_exclusion'.cooldownHours, clamped 24-72) must pass AND admin must call
  -- admin_action('lift-self-exclusion'). self_excluded stays true meanwhile.
...
-- ----------------------------------------------------------------------------
-- Issue 9 (part a): admin_action — atomic adjust-balance + lift-self-exclusion
-- ----------------------------------------------------------------------------
...
begin
  ...
  if p_action = 'adjust-balance' then
    ...
    -- PATCH8 (issue 8): admin lifts self-exclusion only after cooldown elapsed
    v_uid := (p_params->>'uid')::uuid;
    update public.profiles
    set self_excluded = false, self_exclusion_until = null, self_exclusion_lift_pending = false
    where id = v_uid and self_excluded = true
      and (self_exclusion_until is null or now() >= self_exclusion_until)
    returning id into v_row;
    if not found then
      if exists (select 1 from public.profiles where id = v_uid and self_excluded) then
        raise exception 'Cooldown abhi puri nahi hui — baad me try karo';
      end if;
...
-- ----------------------------------------------------------------------------
-- Issue 7: drop direct deposit INSERT policy (request_deposit only)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- Issue 9 (part b): function-surface lockdown + explicit client grants
-- ----------------------------------------------------------------------------
revoke execute on all functions in schema public from public;
revoke execute on all functions in schema public from anon;
revoke execute on all functions in schema public from authenticated;

-- service_role keeps full execute (edge functions run as service_role)
grant execute on all functions in schema public to service_role;
grant execute on all functions in schema public to postgres;

-- CLIENT (authenticated) — only RPCs the UI actually calls, each with
-- internal auth.uid()/is_admin() checks. (Verified against src grep 2026-08-25.)
...
-- trigger function: admin (authenticated) inserts/updates public_links
grant execute on function public.check_link_url()                    to authenticated;

-- CLIENT (anon) — pre-auth reads only (RLS still limits what they can see)
...
--   mfa_check_code, totp_code, rank_for_count, rl_check, v_thread_ok,
--   ledger_backfill, handle_new_user, razorpay_credit, admin_secret
--
-- future functions: deny-by-default
