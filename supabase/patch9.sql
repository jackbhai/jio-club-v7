-- ============================================================================
-- JIO CLUB v7 — PATCH 9 (idempotent; safe to re-run)
-- Final hardening pass (post-patch8 audit findings):
--
--  A) global_chat — legacy table had "Allow all for anon" (ALL) policy:
--     ANY anonymous visitor could read/insert/update/delete. Policy dropped
--     (table now deny-all; data untouched).
--  B) Razorpay key SECRETS were stored in public.settings['payments'] and
--     readable by ANY authenticated user (settings_read). Moved to new
--     public.payment_keys table (RLS: admin-only). Key IDs stay in settings
--     (Razorpay key IDs are public identifiers by design — checkout widget
--     runs in the browser). Edge function v5 reads secrets from payment_keys.
--  C) admin_action: set-role / set-status now refuse self-targets and
--     admin-targets (admin lockout protection).
--
-- NO UI/feature changes (except admin Payments panel now reads/writes the
-- secrets via payment_keys table — same fields, same UX).
-- ============================================================================

-- ----------------------------------------------------------------------------
-- A) Lock down legacy global_chat (was: Allow all for anon)
-- ----------------------------------------------------------------------------
drop policy if exists "Allow all for anon" on public.global_chat;
-- (no replacement policy — table is now inaccessible to all client roles)

-- ----------------------------------------------------------------------------
-- B) payment_keys — admin-only storage for Razorpay key SECRETS
-- ----------------------------------------------------------------------------
create table if not exists public.payment_keys (
  env         text primary key check (env in ('test','live')),
  key_secret  text not null default '',
  updated_at  timestamptz not null default now()
);
alter table public.payment_keys enable row level security;
drop policy if exists "pkeys_admin" on public.payment_keys;
create policy "pkeys_admin" on public.payment_keys for all
using (public.is_admin()) with check (public.is_admin());

-- migrate existing secrets out of settings (one-time, idempotent)
insert into public.payment_keys (env, key_secret)
values
  ('test', coalesce((select value->>'testKeySecret' from public.settings where key='payments'), '')),
  ('live', coalesce((select value->>'liveKeySecret' from public.settings where key='payments'), ''))
on conflict (env) do update set key_secret = excluded.key_secret, updated_at = now();

-- strip secrets from settings (key IDs intentionally remain — public by design)
update public.settings
set value = value - 'testKeySecret' - 'liveKeySecret'
where key = 'payments' and (value ? 'testKeySecret' or value ? 'liveKeySecret');

-- ----------------------------------------------------------------------------
-- C) admin_action (+ self/admin lockout guards on set-role, set-status)
-- ----------------------------------------------------------------------------
...
-- grants for new objects (consistent with patch8 lockdown)
