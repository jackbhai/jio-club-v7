  ...
  return (
    <div>
      <div className="card" style={{ marginBottom: 12, display: 'flex', gap: 10, alignItems: 'flex-start', background: 'linear-gradient(135deg, rgba(124,108,255,0.1), transparent)' }}>
        ...
        <div>
          <div style={{ fontWeight: 900, fontSize: '0.9rem' }}>{t('chat.announcements')}</div>
          <p className="card-sub">{t('chat.announcements_sub')}</p>
        </div>
      </div>
      <div className="card">
        <div className="card-title">
          <Ic n="chat" s={18} />{t('chat.feed')}
          <span className="badge badge-active" style={{ marginLeft: 4 }}><Ic n="radio" s={11} />{t('chat.live')}</span>
        </div>
        <div className="chat-box" ref={boxRef}>
          {!msgs && <div className="spinner"></div>}
          {msgs?.length === 0 && <Empty icon="megaphone" msg={t('chat.no_ann')} />}
          {msgs?.map((m) => (
        ...
        <div style={{ marginTop: 12, padding: '10px 12px', background: 'var(--card-2)', borderRadius: 10, color: 'var(--text-dim)', fontSize: '0.8rem', display: 'flex', gap: 8, alignItems: 'center' }}>
          <Ic n="lock" s={14} />{t('wallet.readonly_feed')}
        </div>
      </div>
    </div>
  );
